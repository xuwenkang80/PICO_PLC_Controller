...
```text
run=1 mode=STEPPER in=0x00 out=0x06 step=1 dir=CW pps=200 ena=1
```
...
- 解析兼容：
  - 文本：`run=1 mode=STEPPER in=0x00 out=0x06 step=1 dir=CW pps=200 ena=1`
  - JSON：`{"run":1,"mode":"STEPPER","in":0,"out":6,"step":1,"dir":"CW","pps":200,"ena":1}`
...
如果 TB6600 的 ENA 需要低电平有效，在固件中把 `kStepperEnableActiveHigh` 改为 `false` 后重新编译。
